...
Item {
    ...
    property string iconSource: Icons.icon("system-suspend-inhibited-symbolic")
    property bool isActive: Caffeine.caffeineActive
    property color activeColor: Qt.rgba(1.0, 0.6, 0.2, 1.0) // Orange-ish for coffee
    signal toggled()
    onToggled: Caffeine.setCaffeine(!Caffeine.caffeineActive)
}
